...
export class AuthService {
  ...
  login(values: any): Observable<boolean> {
    ...
    return this.http.post<AccessToken>(`${APP_SETTINGS.baseUrl}/api/auth/login`, body)
      .pipe(
        tap(accessToken => {
          if (accessToken) {
            this.storageService
              .set('access_token', accessToken, accessToken.expiresIn);
          }
        }),
        map(res => !!res),
  ...
  signup(values: any): Observable<any> {
    ...
    return this.http.post(`${APP_SETTINGS.baseUrl}/api/accounts/register`, body);
  }
